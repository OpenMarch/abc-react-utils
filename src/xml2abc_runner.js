 ...
 * @param {*} options
 */
function xml2abc_runner(files, options) {
    if (!files || files.length === 0)
        fout('no input files');
    for (let file of files) {
        x = path.parse(file)
        if (['.xml', '.musicxml', '.mxl'].indexOf(x.ext) == -1) {
            console.error('*error* ' + file + ' skipped, no .xml')
            continue
        }
        ...
        if (x.ext == '.mxl') {
            const zip = new strzip({ file: file, storeEntries: true });
            zip.on('ready', () => {
            ...
            xmltxt = fs.readFileSync(file);
            doFile(xmltxt, opad, x.name, options);
        }
    }
}

export default xml2abc_runner;
